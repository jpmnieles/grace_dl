...
def objective_function(V, x_c_l, y_c_l, z_c_l, x_c_r, y_c_r, z_c_r,
    ...
    left_rvec = left_rvec.flatten()
    left_tvec = T_clprime[:3,3].flatten()
    right_rvec,_ = cv2.Rodrigues(T_crprime[:3,:3])
    right_rvec = right_rvec.flatten()
    right_tvec = T_crprime[:3,3].flatten()
    ...
    # Loss
    residuals = np.sum((left_rvec-l_rvec)**2 + (left_tvec-l_tvec)**2 
                       + (right_rvec-r_rvec)**2 + (right_tvec-r_tvec)**2)
...
def main(i):
    ...
    l_rvec = np.array([data_df['l_rvec_0'],data_df['l_rvec_1'],data_df['l_rvec_2']])
    l_tvec = np.array([data_df['l_tvec_0'],data_df['l_tvec_1'],data_df['l_tvec_2']])
    r_rvec = np.array([data_df['r_rvec_0'],data_df['r_rvec_1'],data_df['r_rvec_2']])
    r_tvec = np.array([data_df['r_tvec_0'],data_df['r_tvec_1'],data_df['r_tvec_2']])
